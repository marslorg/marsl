...
class TaggedRSS
{
    ...
    public function display(): void
    {
        ...
        if ($this->authentication->moduleReadAllowed("news", $this->role->getGuestRole())) {
            ...
            $items = array();
            $result = $this->db->query("SELECT `news`.`location` AS `location`, `news`.`news` AS `news`, `news`.`teaser` AS `teaser`, `news`.`headline` AS `headline`, `news`.`title` AS `title`, `news`.`postdate` AS `postdate`, `news`.`picture1` AS `picture1`, `news`.`picture2` AS `picture2` FROM `news`
    ...
    private function tagTeaser(string $text, string $tag): string
    {
...
$taggedRSS = ComponentBuilder::buildDependencies()->make('marsl\TaggedRSS');

if ($taggedRSS instanceof TaggedRSS) {
    $taggedRSS->display();
}
