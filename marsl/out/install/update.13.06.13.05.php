...
class Install
{
    ...
    public function startInstall(): void
    {
        ...
        $this->db->query("UPDATE `email` SET `primary`='1'");

        $this->db->query("INSERT INTO `module`(`name`,`file`,`class`) VALUES('Registrierung','register','Register')");
        $this->db->query("INSERT INTO `module`(`name`,`file`,`class`) VALUES('Login','login','Login');");

        $roleID = $this->role->getIDbyName("root");
        $this->role->setModuleRights($roleID, "register", true, true, true, true);
        $this->role->setModuleRights($roleID, "login", true, true, true, true);
        $this->db->close();
    }
}
