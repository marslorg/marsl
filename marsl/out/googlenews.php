...
class GoogleNews {
	...
	public function display() {
		...
		if($auth->moduleReadAllowed("news", $role->getGuestRole())) {
			...
			$items = array();
			$result = $db->query("SELECT `news`.`location` AS `location`, `news`.`news` AS `news`, `news`.`teaser` AS `teaser`, `news`.`headline` AS `headline`, `news`.`title` AS `title`, `news`.`postdate` AS `postdate`, `news`.`picture1` AS `picture1`, `news`.`picture2` AS `picture2`, `news`.`text` AS `text` FROM `news`
			...
			while ($row = $db->fetchArray($result)) {
				...
				$text = htmlentities($row['text'], null, "ISO-8859-1");
