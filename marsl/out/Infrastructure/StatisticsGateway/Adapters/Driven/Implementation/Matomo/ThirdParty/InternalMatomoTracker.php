...
namespace marsl\Infrastructure\StatisticsGateway\Adapters\Driven\Implementation\Matomo\ThirdParty;
 ...
 *
 * @category Matomo
 * @package InternalMatomoTracker
 */

/**
 * InternalMatomoTracker implements the Matomo Tracking Web API.
 *
...
class InternalMatomoTracker
{
     ...
     * InternalMatomoTracker::$URL = 'http://yourwebsite.org/matomo/';
     *
     * @var string
     */
    public static $URL = '';
    ...
    /**
     * Builds a InternalMatomoTracker object, used to track visits, pages and Goal conversions
     ...
     *                         If set, will overwrite InternalMatomoTracker::$URL
     */
     ...
     * InternalMatomoTracker.getAttributionInfo() and that you have JSON encoded via JSON2.stringify()
     *
...
didn't change any existing VisitorId value */
    ...
    /**
     * By default, InternalMatomoTracker will read first party cookies
        ...
        if (empty(self::$URL)) {
            throw new Exception(
                'You must first set the Matomo Tracker URL by calling
                 InternalMatomoTracker::$URL = \'http://your-website.org/matomo/\';'
            );
        }
...
{
    $tracker = new InternalMatomoTracker($idSite);
